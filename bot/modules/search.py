    ...
    if method.startswith("api"):
        ...
        try:
            ...
            if "error" in search_results or search_results["total"] == 0:
                await edit_message(
                    message,
                    f"Tidak ada hasil ditemukan untuk <i>{key}</i>\nSitus Torrent:- <i>{SITES.get(site)}</i>",
                )
                return
            msg = f"<b>Ditemukan {min(search_results['total'], TELEGRAPH_LIMIT)}</b>"
            if method == "apitrend":
                msg += f" <b>hasil trending\nSitus Torrent:- <i>{SITES.get(site)}</i></b>"
            elif method == "apirecent":
                msg += (
                    f" <b>hasil terbaru\nSitus Torrent:- <i>{SITES.get(site)}</i></b>"
                )
        ...
        if total_results == 0:
            ...
            return
        msg = f"<b>Ditemukan {min(total_results, TELEGRAPH_LIMIT)}</b>"
...
async def get_result(search_results, key, message, method):
    telegraph_content = []
    if method == "apirecent":
        msg = "<h4>Hasil Terbaru API</h4>"
    elif method == "apisearch":
        msg = f"<h4>Hasil Pencarian API Untuk {key}</h4>"
    elif method == "apitrend":
        msg = "<h4>Hasil Trending API</h4>"
    else:
        msg = f"<h4>Hasil Pencarian PLUGINS Untuk {key}</h4>"
    for index, result in enumerate(search_results, start=1):
        if method.startswith("api"):
            try:
                ...
                if "torrents" in result.keys():
                    for subres in result["torrents"]:
                        msg += f"<b>Kualitas: </b>{subres['quality']} | <b>Tipe: </b>{subres['type']} | "
                        msg += f"<b>Ukuran: </b>{subres['size']}<br>"
                        if "torrent" in subres.keys():
                            msg += f"<a href='{subres['torrent']}'>Tautan Langsung</a><br>"
                        elif "magnet" in subres.keys():
                            msg += "<b>Bagikan Magnet ke</b> "
                            msg += f"<a href='http://t.me/share/url?url={subres['magnet']}'>Telegram</a><br>"
                    msg += "<br>"
                else:
                    msg += f"<b>Ukuran: </b>{result['size']}<br>"
                    try:
                        msg += f"<b>Seeder: </b>{result['seeders']} | <b>Leecher: </b>{result['leechers']}<br>"
                    except Exception:
                        pass
                    if "torrent" in result.keys():
                        msg += f"<a href='{result['torrent']}'>Tautan Langsung</a><br><br>"
            ...
            msg += f"<b>Ukuran: </b>{get_readable_file_size(result.fileSize)}<br>"
            msg += f"<b>Seeder: </b>{result.nbSeeders} | <b>Leecher: </b>{result.nbLeechers}<br>"
            link = result.fileUrl
            if link.startswith("magnet:"):
                msg += f"<b>Bagikan Magnet ke</b> <a href='http://t.me/share/url?url={quote(link)}'>Telegram</a><br><br>"
            else:
                msg += f"<a href='{link}'>Tautan Langsung</a><br><br>"
    ...
    await edit_message(
        message, f"<b>Membuat</b> {len(telegraph_content)} <b>halaman Telegraph.</b>"
    )
    path = [
        (
            await telegraph.create_page(
                title="Pencarian Torrent Mirror-leech-bot", content=content
            )
        )["path"]
    ...
    if len(path) > 1:
        await edit_message(
            message, f"<b>Mengedit</b> {len(telegraph_content)} <b>halaman Telegraph.</b>"
        )
...
def api_buttons(user_id, method):
    ...
    buttons.data_button("Batal", f"torser {user_id} cancel")
...
async def plugin_buttons(user_id):
    ...
    buttons.data_button("Semua", f"torser {user_id} all plugin")
    buttons.data_button("Batal", f"torser {user_id} cancel")
    return buttons.build_menu(2)
...
async def torrent_search(_, message):
    ...
    if SITES is None and not Config.SEARCH_PLUGINS:
        await send_message(
            message, "Tidak ada tautan API atau PLUGINS pencarian yang ditambahkan untuk fungsi ini"
        )
    elif len(key) == 1 and SITES is None:
        await send_message(message, "Kirimkan kata kunci pencarian beserta perintahnya")
    elif len(key) == 1:
        buttons.data_button("Trending", f"torser {user_id} apitrend")
        buttons.data_button("Terbaru", f"torser {user_id} apirecent")
        buttons.data_button("Batal", f"torser {user_id} cancel")
        button = buttons.build_menu(2)
        await send_message(message, "Kirimkan kata kunci pencarian beserta perintahnya", button)
    elif SITES is not None and Config.SEARCH_PLUGINS:
        buttons.data_button("API", f"torser {user_id} apisearch")
        buttons.data_button("Plugin", f"torser {user_id} plugin")
        buttons.data_button("Batal", f"torser {user_id} cancel")
        button = buttons.build_menu(2)
        await send_message(message, "Pilih alat untuk mencari:", button)
    elif SITES is not None:
        button = api_buttons(user_id, "apisearch")
        await send_message(message, "Pilih situs untuk mencari | API:", button)
    else:
        button = await plugin_buttons(user_id)
        await send_message(message, "Pilih situs untuk mencari | Plugin:", button)
...
async def torrent_search_update(_, query):
    ...
    if user_id != int(data[1]):
        ...
        await edit_message(message, "Pilih situs:", button)
    elif data[2] == "plugin":
        await query.answer()
        button = await plugin_buttons(user_id)
        await edit_message(message, "Pilih situs:", button)
        ...
        await query.answer()
        await edit_message(message, "Pencarian telah dibatalkan!")
