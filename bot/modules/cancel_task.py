...
async def cancel(_, message):
    ...
    if len(msg) > 1:
        ...
        if len(gid) == 6:
            ...
            if task is None:
                await send_message(message, f"GID: <code>{gid}</code> Tidak Ditemukan.")
                return
    elif reply_to_id := message.reply_to_message_id:
        async with task_dict_lock:
            task = task_dict.get(reply_to_id)
        if task is None:
            await send_message(message, "Ini bukan tugas yang aktif!")
            return
    elif len(msg) == 1:
        msg = (
...
def create_cancel_buttons(is_sudo, user_id=""):
    ...
    buttons.data_button(
        "Mengunduh", f"canall ms {MirrorStatus.STATUS_DOWNLOAD} {user_id}"
    )
    buttons.data_button(
        "Mengunggah", f"canall ms {MirrorStatus.STATUS_UPLOAD} {user_id}"
    )
    buttons.data_button("Menyemai", f"canall ms {MirrorStatus.STATUS_SEED} {user_id}")
    buttons.data_button("Memecah", f"canall ms {MirrorStatus.STATUS_SPLIT} {user_id}")
    buttons.data_button("Menggandakan", f"canall ms {MirrorStatus.STATUS_CLONE} {user_id}")
    buttons.data_button(
        "Mengekstrak", f"canall ms {MirrorStatus.STATUS_EXTRACT} {user_id}"
    )
    buttons.data_button(
        "Mengarsipkan", f"canall ms {MirrorStatus.STATUS_ARCHIVE} {user_id}"
    )
    buttons.data_button(
        "Antrean Unduh", f"canall ms {MirrorStatus.STATUS_QUEUEDL} {user_id}"
    )
    buttons.data_button(
        "Antrean Unggah", f"canall ms {MirrorStatus.STATUS_QUEUEUP} {user_id}"
    )
    buttons.data_button(
        "Video Sampel", f"canall ms {MirrorStatus.STATUS_SAMVID} {user_id}"
    )
    buttons.data_button(
        "Konversi Media", f"canall ms {MirrorStatus.STATUS_CONVERT} {user_id}"
    )
    ...
    if is_sudo:
        if user_id:
            ...
            buttons.data_button("Tugas Saya", f"canall user ms {user_id}")
    buttons.data_button("Tutup", f"canall close ms {user_id}")
...
async def cancel_all_buttons(_, message):
    async with task_dict_lock:
        count = len(task_dict)
    if count == 0:
        await send_message(message, "Tidak ada tugas aktif!")
        return
